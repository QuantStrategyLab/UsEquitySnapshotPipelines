...
Yahoo Finance 不需要新密钥，但其公开图表通道不应被视为正式交易行情 API；本系统只将其用于独立交叉校验。Twelve Data 的 Basic 账户提供每日受限额度，密钥仅保存为仓库环境 `tqqq-p1-p3-nonlive` 的 `TWELVE_DATA_API_KEY` secret，绝不放入代码、日志或聊天。
