...
- 完整中文版见 [`operator_runbook.zh-CN.md`](operator_runbook.zh-CN.md)；本节保留在英文文件顶部，方便从当前文件直接找到中文入口。
- 用途：本文档围绕 `Operator Runbook`，用于理解 `UsEquitySnapshotPipelines` 的配置、运行、部署、研究或验收边界。
- 主要覆盖：`Snapshot Profiles`、`Manual Local Build`、`Manual GitHub Actions Build`、`Scheduled Publish`、`Monthly AI Review`。
