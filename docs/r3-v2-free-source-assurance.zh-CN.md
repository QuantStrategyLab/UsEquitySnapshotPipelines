...
另有独立的 [R3-v2 price-only](r3-v2-price-only-assurance.zh-CN.md) 合同（只比较 OHLC、`volume_not_consumed`）；二者 schema / contract_id / 候选身份互不复用，本 full-OHLCV 门槛不会因 price-only 而被放宽。
