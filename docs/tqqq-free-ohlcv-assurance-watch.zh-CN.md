...
每个探针只保留 UTC 采样时间、请求日期窗口、来源响应哈希、状态和按 OHLC 字段的差异分布；不会上传价格、成交量、原始响应、路径或凭据。工作流仅以 GitHub Actions 的只读权限读取自己此前的脱敏工件，生成 `PENDING_T_PLUS_2`、`REVISED_OR_UNSETTLED`、`SETTLED_VERIFIED` 或 `PERSISTENT_CROSS_SOURCE_DISAGREEMENT` 等数据结算状态。
