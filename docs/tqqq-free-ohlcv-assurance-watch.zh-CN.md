...
V9 的手动 P1/P3 研究不会使用可能仍在结算中的最新完成交易日；它固定取最近可用完成交易日向前数的第三个 XNYS 会话（T+2）。这只是在不改变双源、1bp 校验或策略参数的前提下，选择已由本观察机制验证会稳定的输入时点。若 T+2 仍无法通过，研究仍会安全停放，绝不回退到单一来源或自动放宽容差。
