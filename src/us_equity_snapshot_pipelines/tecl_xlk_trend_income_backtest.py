...
def _build_tecl_delever_overlay_history(
    price_history: pd.DataFrame,
    *,
    kind: str,
    symbol: str,
    ...
    if kind == "drawdown":
        ...
        if mode not in {"fixed", "rolling_percentile"}:
            raise ValueError("unsupported TECL volatility delever threshold mode")
        ...
        raise ValueError("unsupported TECL delever overlay kind")
    ...
    for symbol in ("TECL", "XLK"):
        ...
        history = pd.DataFrame(
            {
                ...
                "ma_trend": close.rolling(
                    int(tecl_xlk_trend_income_manifest.default_config["trend_ma_window"])
                ).mean(),
            },
            index=close.index,
        )
        ...
        if symbol == "XLK":
            ...
            volatility_window = int(
                tecl_xlk_trend_income_manifest.default_config["blend_gate_volatility_delever_window"]
            )
            volatility_lookback = int(
                tecl_xlk_trend_income_manifest.default_config["blend_gate_volatility_delever_dynamic_lookback"]
            )
            volatility_percentile = float(
                tecl_xlk_trend_income_manifest.default_config["blend_gate_volatility_delever_dynamic_percentile"]
            )
            volatility_min_periods = min(
                volatility_lookback,
                int(
                    tecl_xlk_trend_income_manifest.default_config["blend_gate_volatility_delever_dynamic_min_periods"]
                ),
            )
            volatility_floor = float(
                tecl_xlk_trend_income_manifest.default_config["blend_gate_volatility_delever_dynamic_floor"]
            )
            volatility_cap = float(
                tecl_xlk_trend_income_manifest.default_config["blend_gate_volatility_delever_dynamic_cap"]
            )
...
def _strategy_kwargs(overrides: Mapping[str, object] | None = None) -> dict[str, object]:
    config = tecl_xlk_trend_income_manifest.default_config
    ...
    kwargs = {
        ...
        "attack_allocation_mode": str(config.get("attack_allocation_mode", "xlk_gate_tiered_blend")),
        "blend_gate_trend_source": str(config.get("blend_gate_trend_source", "XLK")),
        "blend_gate_tecl_weight": float(config.get("blend_gate_tecl_weight", 0.75)),
        "blend_gate_mid_tecl_weight": float(config.get("blend_gate_mid_tecl_weight", 0.65)),
        "blend_gate_active_xlk_weight": float(config.get("blend_gate_active_xlk_weight", 0.20)),
        "blend_gate_defensive_xlk_weight": float(config.get("blend_gate_defensive_xlk_weight", 0.15)),
        ...
        "blend_gate_volatility_delever_symbol": str(config.get("blend_gate_volatility_delever_symbol", "XLK")),
        ...
        "blend_gate_volatility_delever_retention_policy": str(
            config.get("blend_gate_volatility_delever_retention_policy", "tecl_step_rebound_0.25_0.50")
        ),
        ...
        "blend_gate_volatility_delever_redirect_symbol": str(
            config.get("blend_gate_volatility_delever_redirect_symbol", "XLK")
        ),
    }
...
def _execute_rebalance(
    *,
    ...
    income_symbols = tuple(symbol for symbol in MANAGED_SYMBOLS if symbol not in {"TECL", "XLK", "BOXX"})
    sell_order = ("TECL", "XLK", *income_symbols, "BOXX")
    buy_order = (*income_symbols, "TECL", "XLK", "BOXX")
...
def run_backtest(
    ...
    *,
    ...
    chandelier_stop_symbol: str = "XLK",
    chandelier_window: int = 22,
    chandelier_atr_multiple: float = 3.0,
    tecl_delever_overlay_kind: str = "none",
    tecl_delever_overlay_symbol: str | None = None,
    tecl_delever_overlay_window: int | None = None,
    tecl_delever_overlay_fast_window: int | None = None,
    tecl_delever_overlay_slow_window: int | None = None,
    tecl_delever_overlay_threshold: float | None = None,
    tecl_delever_overlay_threshold_mode: str = "fixed",
    tecl_delever_overlay_threshold_lookback: int | None = None,
    tecl_delever_overlay_threshold_percentile: float | None = None,
    tecl_delever_overlay_threshold_min_periods: int | None = None,
    tecl_delever_overlay_threshold_floor: float | None = None,
    tecl_delever_overlay_threshold_cap: float | None = None,
    tecl_delever_overlay_atr_multiple: float | None = None,
    tecl_delever_overlay_retention_ratio: float = 0.0,
    tecl_delever_overlay_redirect_symbol: str = "BOXX",
    tecl_delever_overlay_combine_with_core: bool = False,
    ...
    rsi_threshold = (
        ...
        else float(tecl_xlk_trend_income_manifest.default_config.get("blend_gate_rsi_threshold", 70.0))
    )
    ...
    overlay_kind = _normalize_overlay_kind(tecl_delever_overlay_kind)
    if chandelier_stop_enabled and overlay_kind == "none":
        overlay_kind = "chandelier"
    tecl_delever_enabled = overlay_kind != "none"
    overlay_symbol = _normalize_symbol(tecl_delever_overlay_symbol or chandelier_stop_symbol or "XLK")
    overlay_window = int(tecl_delever_overlay_window or tecl_delever_overlay_slow_window or chandelier_window)
    overlay_fast_window = int(tecl_delever_overlay_fast_window or 10)
    overlay_slow_window = int(tecl_delever_overlay_slow_window or tecl_delever_overlay_window or 30)
    overlay_atr_multiple = float(
        tecl_delever_overlay_atr_multiple if tecl_delever_overlay_atr_multiple is not None else chandelier_atr_multiple
    )
    overlay_retention_ratio = _clamp_ratio(tecl_delever_overlay_retention_ratio)
    overlay_redirect_symbol = _normalize_symbol(tecl_delever_overlay_redirect_symbol or "BOXX")
    if overlay_redirect_symbol not in MANAGED_SYMBOLS:
        expected = ", ".join(MANAGED_SYMBOLS)
        raise ValueError(
            f"unsupported TECL delever redirect symbol {overlay_redirect_symbol!r}; expected one of {expected}"
        )
    if tecl_delever_enabled and not bool(tecl_delever_overlay_combine_with_core):
        strategy_overrides["blend_gate_volatility_delever_enabled"] = False

    delever_history = (
        _build_tecl_delever_overlay_history(
            prices,
            kind=overlay_kind,
            ...
            threshold=tecl_delever_overlay_threshold,
            threshold_mode=tecl_delever_overlay_threshold_mode,
            threshold_lookback=tecl_delever_overlay_threshold_lookback,
            threshold_percentile=tecl_delever_overlay_threshold_percentile,
            threshold_min_periods=tecl_delever_overlay_threshold_min_periods,
            threshold_floor=tecl_delever_overlay_threshold_floor,
            threshold_cap=tecl_delever_overlay_threshold_cap,
            atr_multiple=overlay_atr_multiple,
        )
        if tecl_delever_enabled
    ...
    for as_of in backtest_index:
        ...
        if pd.isna(close_row.get("TECL")) or pd.isna(close_row.get("XLK")):
            continue
        ...
        if "tecl" not in indicators or "xlk" not in indicators:
            continue
        ...
        if core_volatility_delever_triggered:
            tecl_delever_stop_count += 1
        delever_row = (
            delever_history.loc[as_of]
            if tecl_delever_enabled and as_of in delever_history.index
            else pd.Series(dtype=object)
        )
        delever_triggered = bool(delever_row.get("triggered", False)) if not delever_row.empty else False
        if delever_triggered and float(target_values.get("TECL", 0.0) or 0.0) > 0.0:
            tecl_target_value = float(target_values.get("TECL", 0.0))
            retained_value = tecl_target_value * overlay_retention_ratio
            redirected_value = tecl_target_value - retained_value
            target_values["TECL"] = retained_value
            target_values[overlay_redirect_symbol] = (
                float(target_values.get(overlay_redirect_symbol, 0.0) or 0.0) + redirected_value
            )
            tecl_delever_stop_count += 1
        trend_symbol = str(plan.get("trend_symbol", "XLK")).lower()
        ...
        signal_rows.append(
            {
                ...
                "chandelier_stop_enabled": bool(tecl_delever_enabled and overlay_kind == "chandelier"),
                ...
                "tecl_delever_overlay_enabled": bool(tecl_delever_enabled),
                "tecl_delever_overlay_kind": overlay_kind,
                "tecl_delever_overlay_symbol": overlay_symbol,
                "tecl_delever_overlay_window": overlay_window,
                "tecl_delever_overlay_fast_window": delever_row.get("fast_window") if not delever_row.empty else None,
                "tecl_delever_overlay_slow_window": delever_row.get("slow_window") if not delever_row.empty else None,
                "tecl_delever_overlay_threshold": delever_row.get("threshold") if not delever_row.empty else None,
                "tecl_delever_overlay_threshold_mode": delever_row.get("threshold_mode")
                if not delever_row.empty
                else None,
                "tecl_delever_overlay_dynamic_threshold": delever_row.get("dynamic_threshold")
                if not delever_row.empty
                else None,
                "tecl_delever_overlay_dynamic_sample_count": delever_row.get("dynamic_sample_count")
                if not delever_row.empty
                else None,
                "tecl_delever_overlay_dynamic_lookback": delever_row.get("dynamic_lookback")
                if not delever_row.empty
                else None,
                "tecl_delever_overlay_dynamic_percentile": delever_row.get("dynamic_percentile")
                if not delever_row.empty
                else None,
                "tecl_delever_overlay_dynamic_min_periods": delever_row.get("dynamic_min_periods")
                if not delever_row.empty
                else None,
                "tecl_delever_overlay_dynamic_floor": delever_row.get("dynamic_floor")
                if not delever_row.empty
                else None,
                "tecl_delever_overlay_dynamic_cap": delever_row.get("dynamic_cap") if not delever_row.empty else None,
                "tecl_delever_overlay_atr_multiple": overlay_atr_multiple if overlay_kind == "chandelier" else None,
                "tecl_delever_overlay_retention_ratio": overlay_retention_ratio,
                "tecl_delever_overlay_redirect_symbol": overlay_redirect_symbol,
                "tecl_delever_overlay_metric": delever_row.get("metric") if not delever_row.empty else None,
                "tecl_delever_overlay_fast_ma": delever_row.get("fast_ma") if not delever_row.empty else None,
                "tecl_delever_overlay_slow_ma": delever_row.get("slow_ma") if not delever_row.empty else None,
                "tecl_delever_overlay_triggered": delever_triggered,
            }
        )
    ...
    summary["Chandelier Stops"] = float(tecl_delever_stop_count if overlay_kind == "chandelier" else 0.0)
    summary["TECL Delever Stops"] = float(tecl_delever_stop_count)
    return {
        "summary": summary,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Research backtest for tecl_xlk_trend_income.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable the income layer for long-history core TECL/XLK research",
    )
    parser.add_argument(
        "--enable-chandelier-stop",
        action="store_true",
        help=(
            "Enable research-only Chandelier-style TECL delever overlay. "
    ...
    parser.add_argument(
        "--tecl-delever-overlay",
        default="none",
        choices=("none", "chandelier", "drawdown", "volatility", "momentum", "dual_ma"),
        help="Research-only TECL delever overlay family",
    )
    parser.add_argument("--tecl-delever-symbol", help="Symbol used by the TECL delever overlay")
    parser.add_argument("--tecl-delever-window", type=int, help="Lookback window for the TECL delever overlay")
    parser.add_argument("--tecl-delever-fast-window", type=int, help="Fast window for dual_ma overlay")
    parser.add_argument("--tecl-delever-slow-window", type=int, help="Slow window for dual_ma overlay")
    parser.add_argument(
        "--tecl-delever-threshold",
        type=float,
        help="Overlay threshold: negative for drawdown/momentum, annualized ratio for volatility",
    )
    parser.add_argument(
        "--tecl-delever-threshold-mode",
        default="fixed",
        choices=("fixed", "rolling_percentile"),
        help="Threshold mode for volatility overlays",
    )
    parser.add_argument("--tecl-delever-threshold-lookback", type=int, help="Rolling threshold lookback")
    parser.add_argument(
        "--tecl-delever-threshold-percentile",
        type=float,
        help="Rolling threshold percentile, for example 0.90",
    )
    parser.add_argument(
        "--tecl-delever-threshold-min-periods",
        type=int,
        help="Minimum samples required before using a rolling threshold",
    )
    parser.add_argument("--tecl-delever-threshold-floor", type=float, help="Lower bound for rolling threshold")
    parser.add_argument("--tecl-delever-threshold-cap", type=float, help="Upper bound for rolling threshold")
    parser.add_argument(
        "--tecl-delever-atr-multiple",
        type=float,
        help="ATR multiple when --tecl-delever-overlay=chandelier",
    )
    parser.add_argument(
        "--tecl-delever-retention-ratio",
        type=float,
        default=0.0,
        help="Fraction of the TECL target retained when the overlay triggers",
    )
    parser.add_argument(
        "--tecl-delever-redirect-symbol",
        default="BOXX",
        help="Managed symbol receiving the removed TECL target value",
    )
    parser.add_argument(
        "--tecl-delever-combine-with-core",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_backtest(
        ...
        tecl_delever_overlay_kind=args.tecl_delever_overlay,
        tecl_delever_overlay_symbol=args.tecl_delever_symbol,
        tecl_delever_overlay_window=args.tecl_delever_window,
        tecl_delever_overlay_fast_window=args.tecl_delever_fast_window,
        tecl_delever_overlay_slow_window=args.tecl_delever_slow_window,
        tecl_delever_overlay_threshold=args.tecl_delever_threshold,
        tecl_delever_overlay_threshold_mode=args.tecl_delever_threshold_mode,
        tecl_delever_overlay_threshold_lookback=args.tecl_delever_threshold_lookback,
        tecl_delever_overlay_threshold_percentile=args.tecl_delever_threshold_percentile,
        tecl_delever_overlay_threshold_min_periods=args.tecl_delever_threshold_min_periods,
        tecl_delever_overlay_threshold_floor=args.tecl_delever_threshold_floor,
        tecl_delever_overlay_threshold_cap=args.tecl_delever_threshold_cap,
        tecl_delever_overlay_atr_multiple=args.tecl_delever_atr_multiple,
        tecl_delever_overlay_retention_ratio=float(args.tecl_delever_retention_ratio),
        tecl_delever_overlay_redirect_symbol=args.tecl_delever_redirect_symbol,
        tecl_delever_overlay_combine_with_core=bool(args.tecl_delever_combine_with_core),
    )
    ...
    if output_dir is not None:
        ...
        write_json(
            output_dir / "backtest_config.json",
            {
                ...
                "chandelier_window": int(args.chandelier_window),
                "chandelier_atr_multiple": float(args.chandelier_atr_multiple),
                "tecl_delever_overlay": args.tecl_delever_overlay,
                "tecl_delever_symbol": args.tecl_delever_symbol,
                "tecl_delever_window": args.tecl_delever_window,
                "tecl_delever_fast_window": args.tecl_delever_fast_window,
                "tecl_delever_slow_window": args.tecl_delever_slow_window,
                "tecl_delever_threshold": args.tecl_delever_threshold,
                "tecl_delever_threshold_mode": args.tecl_delever_threshold_mode,
                "tecl_delever_threshold_lookback": args.tecl_delever_threshold_lookback,
                "tecl_delever_threshold_percentile": args.tecl_delever_threshold_percentile,
                "tecl_delever_threshold_min_periods": args.tecl_delever_threshold_min_periods,
                "tecl_delever_threshold_floor": args.tecl_delever_threshold_floor,
                "tecl_delever_threshold_cap": args.tecl_delever_threshold_cap,
                "tecl_delever_atr_multiple": args.tecl_delever_atr_multiple,
                "tecl_delever_retention_ratio": float(args.tecl_delever_retention_ratio),
                "tecl_delever_redirect_symbol": args.tecl_delever_redirect_symbol,
                "tecl_delever_combine_with_core": bool(args.tecl_delever_combine_with_core),
            },
        )
