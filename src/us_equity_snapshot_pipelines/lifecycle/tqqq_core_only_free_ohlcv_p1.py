"""Candidate-bound, two-source free OHLCV P1 input for TQQQ P2 v8.
...
def _policy(symbol: str, cutoff: str) -> MultiSourceDailyBarPolicy:
    return MultiSourceDailyBarPolicy(
        scope_id=f"tqqq_core_only_p2_v8_free_ohlcv_relative_benchmark:{symbol.lower()}",
        symbol=symbol,
        date_cutoff=cutoff,
...
def _assurance_member(*, cutoff: str, reports: Mapping[str, object], bars: Mapping[str, object]) -> dict[str, object]:
    ...
    for symbol in _UNIVERSE:
        ...
        expected = {
            ...
            "policy_sha256": _policy(symbol, cutoff).policy_sha256,
            "scope_id": f"tqqq_core_only_p2_v8_free_ohlcv_relative_benchmark:{symbol.lower()}",
            "symbol": symbol,
            "date_cutoff": cutoff,
...
def validate_tqqq_core_only_free_ohlcv_assurance(value: object, *, bars: Mapping[str, object], cutoff: str) -> dict[str, dict[str, str]]:
...
def _root_payload(binding: object, manifest: object, bars: object, assurance: object) -> tuple[dict[str, object], dict[str, object], dict[str, object], dict[str, object]]:
    ...
    frozen = validate_tqqq_core_only_p1_binding_for_contract(binding, P2_V8_CONTRACT)
    validated_manifest = validate_tqqq_core_only_input_manifest(manifest, frozen, contract=P2_V8_CONTRACT)
    ...
    if not isinstance(bars.get("symbols"), Mapping) or bars.get("schema_version") != _BARS_SCHEMA or set(bars["symbols"]) != set(_UNIVERSE):
        raise TqqqCoreOnlyFreeOhlcvP1Error("invalid free OHLCV input root")
    expected = expected_tqqq_core_only_sessions_for_contract(P2_V8_CONTRACT, date_cutoff=frozen["data_identity"]["date_cutoff"])
    for symbol in _UNIVERSE:
...
def validate_tqqq_core_only_free_ohlcv_input_payload(value: object) -> str:
    ...
    if not isinstance(value, Mapping) or set(value) != {"binding", "input_manifest", "bars", "assurance"}:
        raise TqqqCoreOnlyFreeOhlcvP1Error("invalid free OHLCV input root")
    binding, manifest, bars, assurance = _root_payload(value["binding"], value["input_manifest"], value["bars"], value["assurance"])
    cutoff = str(binding["data_identity"]["date_cutoff"])
    snapshots = validate_tqqq_core_only_free_ohlcv_assurance(assurance, bars=bars["symbols"], cutoff=cutoff)
    expected_sources = {
    ...
    if actual_sources != expected_sources or set(actual_sources) != tqqq_core_only_expected_source_ids(P2_V8_CONTRACT):
...
def publish_tqqq_core_only_free_ohlcv_p1_inputs(observer: TqqqCoreOnlyFreeOhlcvObserver, *, output_root: str | Path, observed_at: str, producer: Mapping[str, object], date_cutoff: str) -> dict[str, object]:
    """Publish V8 only after both free sources cover and agree on every OHLC session."""
    ...
    binding = build_tqqq_core_only_p1_binding_for_contract(P2_V8_CONTRACT, date_cutoff=cutoff)
    expected = expected_tqqq_core_only_sessions_for_contract(P2_V8_CONTRACT, date_cutoff=cutoff)
    ...
    for symbol in _UNIVERSE:
        try:
            ...
            report = assess_multisource_daily_bars(_policy(symbol, cutoff), observations)
    ...
    assurance = _assurance_member(cutoff=cutoff, reports=reports, bars=symbols)
    bars_bytes, assurance_bytes = _canonical(bars), _canonical(assurance)
    snapshots = validate_tqqq_core_only_free_ohlcv_assurance(assurance, bars=symbols, cutoff=cutoff)
    ...
    try:
        ...
        (temporary / "binding.json").write_bytes(canonical_tqqq_core_only_p1_binding_bytes_for_contract(binding, P2_V8_CONTRACT))
        (temporary / "bars.json").write_bytes(bars_bytes)
        (temporary / "assurance.json").write_bytes(assurance_bytes)
        (temporary / "manifest.json").write_bytes(manifest_bytes)
        result = verify_tqqq_core_only_free_ohlcv_p1_input_root(temporary)
...
def verify_tqqq_core_only_free_ohlcv_p1_input_root(output_root: str | Path) -> str:
    """Verify one V8 P1 root without provider or credential access."""
    ...
    try:
        ...
        if members["binding.json"] != canonical_tqqq_core_only_p1_binding_bytes_for_contract(parsed["binding"], P2_V8_CONTRACT) or members["bars.json"] != _canonical(parsed["bars"]) or members["assurance.json"] != _canonical(parsed["assurance"]) or members["manifest.json"] != canonical_research_input_manifest_bytes(parsed["input_manifest"]):
            raise ValueError
        return validate_tqqq_core_only_free_ohlcv_input_payload(parsed)
