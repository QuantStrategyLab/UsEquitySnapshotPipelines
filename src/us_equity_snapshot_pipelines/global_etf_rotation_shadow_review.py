...
from .contracts import GLOBAL_ETF_ROTATION_PROFILE, SOURCE_PROJECT

SHADOW_REVIEW_ARTIFACT_SCHEMA_VERSION = "global_etf_shadow_review_artifact.v1"
RUNTIME_SHADOW_REVIEW_SCHEMA_VERSION = "global_etf_shadow_review.v1"
...
def extract_shadow_review_rows(payload: Mapping[str, Any]) -> ShadowReviewExtraction:
    ...
    schema_version = str(
        diagnostics.get("global_etf_shadow_review_schema_version") or RUNTIME_SHADOW_REVIEW_SCHEMA_VERSION
    ).strip()
    row_fields_raw = diagnostics.get("global_etf_shadow_review_row_fields") or SHADOW_REVIEW_ROW_FIELDS
    ...
    rows_raw = diagnostics.get("global_etf_shadow_review_rows") or ()
    if not isinstance(rows_raw, (list, tuple)):
        raise ValueError("global_etf_shadow_review_rows must be a list")
...
def build_shadow_review_artifacts(
    ...
    *,
    ...
    prefix = str(profile or GLOBAL_ETF_ROTATION_PROFILE).strip() or GLOBAL_ETF_ROTATION_PROFILE
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build Global ETF shadow review CSV/JSON artifacts from runtime diagnostics."
    )
    parser.add_argument("--diagnostics-json", required=True, help="Runtime diagnostics JSON or operator review JSON")
