...
def _lexically_normalized_snapshot_dir(
...
def _open_snapshot_directory(snapshot_dir: str | Path) -> int:
    normalized = _lexically_normalized_snapshot_dir(snapshot_dir)
    try:
        return os.open(
            normalized,
...
def _read_snapshot_member(directory_fd: int, name: str) -> bytes:
    ...
    try:
        ...
        if initial.st_size < 0 or initial.st_size > _MAX_SNAPSHOT_MEMBER_BYTES:
        ...
        while True:
            chunk = os.read(
                ...
                min(
                    1024 * 1024,
                    _MAX_SNAPSHOT_MEMBER_BYTES - received + 1,
                ),
            )
            if not chunk:
                break
            received += len(chunk)
            if received > _MAX_SNAPSHOT_MEMBER_BYTES:
                _invalid("snapshot member exceeds size limit")
            chunks.append(chunk)
        final = os.fstat(descriptor)
        if final.st_size != initial.st_size or received != initial.st_size:
...
def _verify_snapshot_fd(
    ...
    *,
    ...
    members = {name: _read_snapshot_member(directory_fd, name) for name in tqqq_r1_snapshot.OUTPUT_FILENAMES}
    ...
    try:
        sums = tqqq_r1_snapshot._parse_json_object(
            members["sha256sums.json"],
            "sha256sums",
        )
        manifest = tqqq_r1_snapshot._parse_json_object(
            members["manifest.json"],
            "manifest",
        )
        validation = tqqq_r1_snapshot._parse_json_object(
            members["validation.json"],
            "validation",
        )
        if set(sums) != {
            "prices.csv",
...
def load_verified_trusted_snapshot_package(
    ...
    *,
    ...
    try:
        try:
            _, snapshot_members = _verify_snapshot_fd(
                snapshot_fd,
        ...
        try:
            ...
            raise TrustedSnapshotPackageError(
                "invalid verified snapshot prices",
            ) from exc
