"""Descriptor-backed local verification boundary for TQQQ R1 snapshot evidence."""
...
try:
    import fcntl
except ImportError:  # pragma: no cover - unsupported platforms fail closed when loading.
...
class TrustedSnapshotPackage:
    """Verified evidence and one owned directory descriptor; never reopens member paths."""
    ...
    def close(self) -> None:
        """Release the sole owned descriptor; repeated close calls are harmless."""
    ...
    def read_snapshot_member(self, name: str) -> bytes:
        """Return the already verified bytes without reopening a mutable filesystem path."""
...
def _safe_open_flags(*, directory: bool = False, write: bool = False) -> int:
    ...
    return flags | no_follow | non_blocking | directory_flag


def _read_regular(path: str | Path, name: str) -> bytes:
    """Open one evidence file once, admit its descriptor, then return those exact bytes."""
    try:
...
def read_strict_json(payload: bytes, name: str) -> object:
    """Decode finite, duplicate-free JSON with normalized untrusted-input errors."""
...
def write_strict_json(path: str | Path, payload: object) -> None:
    """Write finite canonical JSON without following a raced destination symlink."""
    try:
...
def _stable_snapshot_dir(snapshot_dir: str | Path) -> tuple[int, Path]:
    try:
        descriptor = os.open(os.fspath(snapshot_dir), _safe_open_flags(directory=True))
    except OSError as exc:
    ...
    try:
        if sys.platform.startswith("linux") and Path("/proc/self/fd").is_dir():
            display_path = Path(f"/proc/self/fd/{descriptor}")
        elif sys.platform == "darwin" and fcntl is not None and hasattr(fcntl, "F_GETPATH"):
            display_path = Path(fcntl.fcntl(descriptor, fcntl.F_GETPATH, b"\0" * 1024).split(b"\0", 1)[0].decode())
        else:
            _invalid("stable snapshot directory capability is unavailable")
    except (OSError, UnicodeDecodeError, TrustedSnapshotPackageError) as exc:
        os.close(descriptor)
...
def load_verified_trusted_snapshot_package(
    ...
    *,
    ...
    """Verify anchored local evidence and return a non-copyable stable trusted package."""
    if not all(
        _is_sha256(value)
        for value in (expected_snapshot_manifest_sha256, expected_package_manifest_sha256, expected_receipt_sha256)
    ):
        _invalid("invalid expected evidence hash")
    snapshot_fd, stable_snapshot_dir = _stable_snapshot_dir(snapshot_dir)
    try:
        try:
        ...
        try:
            ...
            snapshot_sessions = {row["session"] for row in csv.DictReader(StringIO(snapshot_members["prices.csv"].decode("utf-8")))}
