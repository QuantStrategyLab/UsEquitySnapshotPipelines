...
@pytest.mark.parametrize("decimal", [".5", "0", "1.0", "01", "1e2", "NaN", "Infinity", "9" * 33])
...
def test_strict_readback_rejects_missing_root_root_file_nonregular_and_oversize(tmp_path: Path) -> None:
    path, bindings = _write_package(tmp_path)
    ...
    path.write_bytes(b"x" * (snapshot.MAX_READ_BYTES + 1))
    with pytest.raises(snapshot.SnapshotValidationError):
