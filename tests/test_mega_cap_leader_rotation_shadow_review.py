...
from us_equity_snapshot_pipelines.mega_cap_leader_rotation_shadow_review import (
...
def _diagnostics_payload() -> dict[str, object]:
    return {
        ...
        "diagnostics": {
            "leader_rotation_shadow_review_schema_version": "russell_top50_shadow_review.v1",
            "leader_rotation_shadow_review_row_fields": ROW_FIELDS,
            "leader_rotation_shadow_review_rows": [
                {
                    "schema_version": "russell_top50_shadow_review.v1",
                    "active_variant": "blend_top2_50_top4_50",
                    "shadow_variant": "top4_baseline",
                    "selected_count": 4,
                    "realized_stock_weight": 1.0,
                    "safe_haven_weight": 0.0,
                    "turnover_delta_vs_active": 0.25,
                    "largest_increase_symbol": "AAPL",
                    "largest_increase_delta": 0.125,
                    "largest_decrease_symbol": "META",
                    "largest_decrease_delta": -0.125,
                    "review_note": "active=blend_top2_50_top4_50 shadow=top4_baseline turnover_delta=25.00%",
...
def test_extract_shadow_review_rows_accepts_release_summary_and_filters_allowed_fields() -> None:
    ...
    assert extracted.schema_version == "russell_top50_shadow_review.v1"
    ...
    assert extracted.rows[0]["shadow_variant"] == "top4_baseline"


def test_build_shadow_review_artifacts_writes_csv_json_and_manifest(tmp_path: Path) -> None:
    diagnostics_path = tmp_path / "release_status_summary.json"
    ...
    assert manifest["profile"] == "russell_top50_leader_rotation"
    assert manifest["snapshot_as_of"] == "2026-06-30"
    assert manifest["row_count"] == 1
    assert manifest["shadow_review_schema_version"] == "russell_top50_shadow_review.v1"
    assert "sha256" in manifest["artifacts"]["csv"]
    assert payload["rows"][0]["shadow_variant"] == "top4_baseline"


def test_shadow_review_rejects_sensitive_review_note() -> None:
    payload = _diagnostics_payload()
    rows = payload["diagnostics"]["leader_rotation_shadow_review_rows"]
    rows[0]["review_note"] = "account=U123 active=blend"
...
def test_shadow_review_cli_writes_outputs(tmp_path: Path) -> None:
    ...
    assert (output_dir / "russell_top50_leader_rotation_shadow_review_manifest.json").exists()
