...
- `auto` 是默认 provider：先由 self-hosted Codex 路径读取 monthly issue、回帖审计结果，并可直接为低风险修复开 PR；如果 Codex 准备或执行失败且 bridge 配置了 `OPENAI_API_KEY`，则回落到 OpenAI 审阅；如果 API key 没配置则明确失败。
